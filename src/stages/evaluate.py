...
def evaluate(config_path: Text) -> None:
    ...
    train_prediction = model.predict(train_data[numerical_features + categorical_features])
    test_prediction = model.predict(test_data[numerical_features + categorical_features])
